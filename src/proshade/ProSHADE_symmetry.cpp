...
std::vector< proshade_double* > ProSHADE_internal_data::ProSHADE_data::getCyclicSymmetriesList ( ProSHADE_settings* settings )
{
    ...
    std::vector< proshade_double* > allPeaks  = ProSHADE_internal_peakSearch::getAllPeaksNaive ( this->getInvSO3Coeffs (), this->getMaxBand() * 2, settings->peakNeighbours, settings->noIQRsFromMedianNaivePeak );
    ...
    for ( proshade_signed iter = static_cast<proshade_unsign> ( peakGroupsBoundaries.size() - 1 ); iter >= 0; iter-- )
    {
        ...
        detectedCSymmetries                   = ProSHADE_internal_symmetry::findPeaksCSymmetry ( &symPeaks, settings->verbose, this->getMaxBand(), settings->symMissPeakThres, settings->axisErrTolerance, this );
...
std::vector< proshade_double* > ProSHADE_internal_symmetry::getPeaksAngleAxisPositions ( std::vector< proshade_double* > allPeaks, proshade_unsign verbose )
{
    ...
    ProSHADE_internal_misc::checkMemoryAllocation ( rotMat, "ProSHADE_symmetry.cpp", 98, "getPeaksAngleAxisPositions()" );
    ...
    for ( proshade_unsign peakIter = 0; peakIter < static_cast<proshade_unsign> ( allPeaks.size() ); peakIter++ )
    {
        ...
        ProSHADE_internal_misc::checkMemoryAllocation ( hlpP, "ProSHADE_symmetry.cpp", 109, "getPeaksAngleAxisPositions()" );
    ...
    if ( ret.size() < 1 )
    {
        ProSHADE_internal_messages::printWarningMessage ( verbose, "!!! ProSHADE WARNING !!! Failed to detect any symmetries. There are no reasonable peaks in the self-rotation map. If you believe there should be some symmetry, you can try decreasing the resolution or changing the peak IQR threshold.", "WS00028" );
    }
   ...
   *dist                                      = 999.9;
    ...
    for ( proshade_unsign gr1It = 0; gr1It < static_cast<proshade_unsign> ( grp.size() ); gr1It++ )
    {
        for ( proshade_unsign gr2It = 1; gr2It < static_cast<proshade_unsign> ( grp.size() ); gr2It++ )
        {
            ...
            {
                //============================ Avoid very small angle distances as they would just take time (the hardcoded value would be problem for C700 and larger symmetries...
...
void ProSHADE_internal_symmetry::addZeroPeakToGroups ( std::vector< std::vector< proshade_unsign > >& grpsVec, std::vector< proshade_double* >& peaks )
{
    ...
    for ( proshade_unsign iter = 0; iter < static_cast<proshade_unsign> ( grpsVec.size() ); iter++ )
    {
        ...
        ProSHADE_internal_misc::checkMemoryAllocation ( hlpP, "ProSHADE_symmetry.cpp", 470, "addZeroPeakToGroups()" );
    ...
    for ( proshade_unsign xIt = 0; xIt < ( dataObj->getMaxBand() * 2 ); xIt++ )
    {
        for ( proshade_unsign yIt = 0; yIt < ( dataObj->getMaxBand() * 2 ); yIt++ )
        {
            for ( proshade_unsign zIt = 0; zIt < ( dataObj->getMaxBand() * 2 ); zIt++ )
            {
                ...
                ProSHADE_internal_maths::getEulerZXZFromSOFTPosition ( dataObj->getMaxBand(), static_cast<proshade_signed> ( xIt ), static_cast<proshade_signed> ( yIt ), static_cast<proshade_signed> ( zIt ), &euA, &euB, &euG );
...
bool ProSHADE_internal_symmetry::completeMissingCSymmetry ( ProSHADE_internal_data::ProSHADE_data* dataObj, proshade_unsign fold, std::vector< proshade_unsign >* grp, std::vector< proshade_double* >* peaks, std::vector< proshade_unsign >* missingPeaks, std::vector< proshade_double >* expectedAngles, std::vector< proshade_unsign >* matchedPeaks, proshade_double axErrTolerance, proshade_unsign verbose )
{
    ...
    for ( proshade_unsign misPkIt = 0; misPkIt < static_cast<proshade_unsign> ( missingPeaks->size() ); misPkIt++ )
    {
        ...
        if ( misHeight != 0.0 )
        {
            ...
            proshade_double* hlpP             = new proshade_double [5];
            ProSHADE_internal_misc::checkMemoryAllocation ( hlpP, "ProSHADE_symmetry.cpp", 834, "completeMissingCSymmetry()" );
...
void ProSHADE_internal_symmetry::findSymmetryUsingFold ( ProSHADE_internal_data::ProSHADE_data* dataObj, std::vector< proshade_unsign >* angsToTry, std::vector< proshade_unsign >* grp, std::vector< proshade_double* >* peaks, std::vector< std::vector< proshade_unsign > >* ret, std::vector< proshade_unsign >* testedAlready, proshade_double axErrTolerance, proshade_double missPeakThres, proshade_unsign verbose )
{
    ...
    for ( proshade_unsign fIt = 0; fIt < static_cast<proshade_unsign> ( angsToTry->size() ); fIt++ )
    {
        ...
        matchedPeaks.clear                    ( );
        missingPeaks.clear                    ( );
        proshade_unsign consecMatches         = ProSHADE_internal_symmetry::checkExpectedAgainstFound ( *grp, *peaks, &expectedAngles, &matchedPeaks, &missingPeaks, axErrTolerance );
        ...
        else
        {
            if ( ( static_cast<proshade_double> ( matchedPeaks.size() ) / static_cast<proshade_double> ( angsToTry->at(fIt) ) ) >= ( 1.0 - missPeakThres ) )
            {
                ...
                if ( ProSHADE_internal_symmetry::completeMissingCSymmetry ( dataObj, angsToTry->at(fIt), grp, peaks, &missingPeaks, &expectedAngles, &matchedPeaks, axErrTolerance, verbose ) )
                {
                    ProSHADE_internal_symmetry::saveDetectedCSymmetry ( angsToTry->at(fIt), &matchedPeaks, ret, verbose );
                }
            }
...
void ProSHADE_internal_symmetry::printSymmetryCompletion ( proshade_unsign noSyms, proshade_unsign verbose )
{
    ...
    if ( noSyms < 1 )
    {
        ProSHADE_internal_messages::printWarningMessage ( verbose, "!!! ProSHADE WARNING !!! Failed to detect any symmetries. If you believe there should be one, you can try decreasing the resolution or checking that the map is centred on the centry of symmetry (or use map centering option in ProSHADE).", "WS00029" );
    }
...
void ProSHADE_internal_symmetry::saveAllCSymmetries ( std::vector< std::vector< proshade_unsign > > detected, std::vector< proshade_double* > peaks, std::vector< proshade_double* >* ret, proshade_double axErr )
{
    ...
    for ( proshade_unsign symIt = 0; symIt < static_cast<proshade_unsign> ( detected.size() ); symIt++ )
    {
        ...
        ProSHADE_internal_misc::checkMemoryAllocation ( hlpP, "ProSHADE_symmetry.cpp", 953, "saveAllCSymmetries()" );
...
bool ProSHADE_internal_symmetry::isSymmetrySame ( std::vector< proshade_double* >* ret, proshade_double* sym, proshade_double simThres )
{
    ...
    for ( proshade_unsign symIt = 0; symIt < static_cast<proshade_unsign> ( ret->size() ); symIt++ )
    {
        ...
        {
            //================================ Is axis the same?
            dotProduct                        = ProSHADE_internal_maths::computeDotProduct ( &ret->at(symIt)[1], &ret->at(symIt)[2], &ret->at(symIt)[3], &sym[1], &sym[2], &sym[3] );
...
std::vector< proshade_double* > ProSHADE_internal_data::ProSHADE_data::getDihedralSymmetriesList ( ProSHADE_settings* settings, std::vector< proshade_double* >* CSymList )
{
    ...
    for ( proshade_unsign ax1 = 0; ax1 < static_cast<proshade_unsign> ( CSymList->size() ); ax1++ )
    {
        for ( proshade_unsign ax2 = 0; ax2 < static_cast<proshade_unsign> ( CSymList->size() ); ax2++ )
        {
            ...
            dotProduct                        = ProSHADE_internal_maths::computeDotProduct ( &CSymList->at(ax1)[1], &CSymList->at(ax1)[2], &CSymList->at(ax1)[3], &CSymList->at(ax2)[1], &CSymList->at(ax2)[2], &CSymList->at(ax2)[3] );
...
void ProSHADE_internal_symmetry::saveDSymmetry ( std::vector< proshade_double* >* ret, std::vector< proshade_double* >* CSymList, proshade_unsign axisOne, proshade_unsign axisTwo )
{
    ...
    ProSHADE_internal_misc::checkMemoryAllocation ( hlpP, "ProSHADE_symmetry.cpp", 1088, "saveDSymmetry()" );
...
std::vector< proshade_double* > ProSHADE_internal_data::ProSHADE_data::getTetrahedralSymmetriesList ( ProSHADE_settings* settings, std::vector< proshade_double* >* CSymList )
{
    ...
    if ( ProSHADE_internal_symmetry::detectTetrahedralSymmetry ( CSymList, settings->axisErrTolerance * 2.0 ) )
    {
        ...
        if ( ret.size() != 4 ) { ProSHADE_internal_messages::printWarningMessage ( settings->verbose, "!!! ProSHADE WARNING !!! Failed to detect some of the polyhedral symmetries, while detecting the correct dihedral angles.", "WS00030" ); return ( ret ); }
        
        ProSHADE_internal_symmetry::findTetra3C2s ( CSymList, &ret, settings->axisErrTolerance * 2.0, this, settings->verbose );
        if ( ret.size() != 7 ) { ProSHADE_internal_messages::printWarningMessage ( settings->verbose, "!!! ProSHADE WARNING !!! Failed to detect some of the polyhedral symmetries, while detecting the correct dihedral angles.", "WS00030" ); return ( ret ); }
    }
...
std::vector < proshade_double* > ProSHADE_internal_symmetry::findMissingAxisPoints ( proshade_double xVal, proshade_double yVal, proshade_double zVal, ProSHADE_internal_data::ProSHADE_data* dataObj, proshade_double axErr )
{
    ...
    proshade_double* rotMat                   = new proshade_double [9];
    ProSHADE_internal_misc::checkMemoryAllocation ( rotMat, "ProSHADE_symmetry.cpp", 1374, "missingAxesHeight()" );
        ...
        for ( proshade_unsign yIt = 0; yIt < ( dataObj->getMaxBand() * 2 ); yIt++ )
        {
            for ( proshade_unsign zIt = 0; zIt < ( dataObj->getMaxBand() * 2 ); zIt++ )
            {
                     ...
                     ( ( ( zPk - axErr ) < zVal ) && ( ( zPk + axErr ) > zVal ) ) )
                {
                    ...
                    ProSHADE_internal_misc::checkMemoryAllocation ( hlpArr, "ProSHADE_symmetry.cpp", 1411, "missingAxesHeight()" );
                    hlpArr[0]                 = anglPk + M_PI;
                    hlpArr[1]                 = pow( dataObj->getInvSO3Coeffs()[arrIndex][0], 2.0 ) + pow( dataObj->getInvSO3Coeffs()[arrIndex][1], 2.0 );
...
void ProSHADE_internal_symmetry::saveMissingAxisNewOnly ( std::vector< proshade_double* >* axVec, proshade_double axX, proshade_double axY, proshade_double axZ, proshade_double height, proshade_unsign fold, proshade_double axErr )
{
    ...
    ProSHADE_internal_misc::checkMemoryAllocation ( hlpSym, "ProSHADE_symmetry.cpp", 1521, "saveMissingAxisNewOnly()" );
...
void ProSHADE_internal_symmetry::searchMissingSymmetrySpace ( ProSHADE_internal_data::ProSHADE_data* dataObj, std::vector< proshade_double* >* CSymList, std::vector< proshade_unsign >* grp, std::vector< proshade_double* >* hlpVec, proshade_double axErr, proshade_double angle, proshade_unsign fold, proshade_double groupAvg )
{
    ...
    ProSHADE_internal_misc::checkMemoryAllocation ( symHlp, "ProSHADE_symmetry.cpp", 1310, "findMissingAxes()" );
...
bool ProSHADE_internal_symmetry::testGroupAgainstGroup ( std::vector< proshade_double* >* GrList1, std::vector< proshade_unsign >* grp1, std::vector< proshade_double* >* GrList2, std::vector< proshade_unsign >* grp2, proshade_double angle, proshade_double axErr )
{
    ...
    for ( proshade_unsign g1It = 0; g1It < static_cast<proshade_unsign> ( grp1->size() ); g1It++ )
    {
        ...
        {
            //================================ Find the angle
            dotProduct                        = ProSHADE_internal_maths::computeDotProduct ( &GrList1->at(grp1->at(g1It))[1], &GrList1->at(grp1->at(g1It))[2], &GrList1->at(grp1->at(g1It))[3], &GrList2->at(grp2->at(g2It))[1], &GrList2->at(grp2->at(g2It))[2], &GrList2->at(grp2->at(g2It))[3] );
...
std::vector< proshade_double* > ProSHADE_internal_data::ProSHADE_data::getOctahedralSymmetriesList ( ProSHADE_settings* settings, std::vector< proshade_double* >* CSymList )
{
    ...
    if ( ProSHADE_internal_symmetry::detectOctahedralSymmetry ( CSymList, settings->axisErrTolerance * 2.0 ) )
    {
        ...
        if ( ret.size() != 3 ) { ProSHADE_internal_messages::printWarningMessage ( settings->verbose, "!!! ProSHADE WARNING !!! Failed to detect some of the polyhedral symmetries, while detecting the correct dihedral angles.", "WS00030" ); return ( ret ); }
        ...
        if ( ret.size() != 13 ) { ProSHADE_internal_messages::printWarningMessage ( settings->verbose, "!!! ProSHADE WARNING !!! Failed to detect some of the polyhedral symmetries, while detecting the correct dihedral angles.", "WS00030" ); return ( ret ); }
    }
...
bool ProSHADE_internal_symmetry::detectOctahedralSymmetry ( std::vector< proshade_double* >* CSymList, proshade_double axErr )
{
    ...
    for ( proshade_unsign c4 = 0; c4 < static_cast<proshade_unsign> ( C4List.size() ); c4++ )
    {
        for ( proshade_unsign cSym = 0; cSym < static_cast<proshade_unsign> ( CSymList->size() ); cSym++ )
        {
            ...
            dotProduct                        = ProSHADE_internal_maths::computeDotProduct ( &CSymList->at(C4List.at(c4))[1], &CSymList->at(C4List.at(c4))[2], &CSymList->at(C4List.at(c4))[3], &CSymList->at(cSym)[1], &CSymList->at(cSym)[2], &CSymList->at(cSym)[3] );
...
void ProSHADE_internal_symmetry::addAxisUnlessSame ( proshade_unsign fold, proshade_double axX, proshade_double axY, proshade_double axZ, proshade_double axHeight, std::vector< proshade_double* >* prosp, proshade_double axErr )
{
    //======================================== Initialise variables
    proshade_double* symHlp                   = new proshade_double[6];
    ProSHADE_internal_misc::checkMemoryAllocation ( symHlp, "ProSHADE_symmetry.cpp", 2266, "addAxisUnlessSame()" );
...
void ProSHADE_internal_symmetry::checkFittingAxisDualAndSave ( std::vector< proshade_unsign >* retGroup, std::vector< proshade_double* >* ret, proshade_unsign fold, proshade_double axX, proshade_double axY, proshade_double axZ, proshade_double groupAvg, std::vector< proshade_double* >* prosp, proshade_double axErr, proshade_unsign noMatchesG1, proshade_double angle1, proshade_unsign noMatchesG2, proshade_double angle2, ProSHADE_internal_data::ProSHADE_data* dataObj )
{
    ...
    for ( proshade_unsign rIt = 0; rIt < static_cast<proshade_unsign> ( retGroup->size() ); rIt++ )
    {
        dotProd                               = ProSHADE_internal_maths::computeDotProduct ( &ret->at(retGroup->at(rIt))[1], &ret->at(retGroup->at(rIt))[2], &ret->at(retGroup->at(rIt))[3], &axX, &axY, &axZ );
...
std::vector< proshade_double* > ProSHADE_internal_data::ProSHADE_data::getIcosahedralSymmetriesList ( ProSHADE_settings* settings, std::vector< proshade_double* >* CSymList )
{
    ...
    if ( ProSHADE_internal_symmetry::detectIcosahedralSymmetry ( CSymList, settings->axisErrTolerance * 2.0 ) )
    {
        ...
        if ( ret.size() != 6 ) { ProSHADE_internal_messages::printWarningMessage ( settings->verbose, "!!! ProSHADE WARNING !!! Failed to detect some of the polyhedral symmetries, while detecting the correct dihedral angles.", "WS00030" ); return ( ret ); }

        ProSHADE_internal_symmetry::findIcos10C3s ( CSymList, &ret, settings->axisErrTolerance * 2.0, this, settings->verbose );
        if ( ret.size() != 16 ) { ProSHADE_internal_messages::printWarningMessage ( settings->verbose, "!!! ProSHADE WARNING !!! Failed to detect some of the polyhedral symmetries, while detecting the correct dihedral angles.", "WS00030" ); return ( ret ); }

        ProSHADE_internal_symmetry::findIcos15C2s ( CSymList, &ret, settings->axisErrTolerance * 2.0, this, settings->verbose );
        if ( ret.size() != 31 ) { ProSHADE_internal_messages::printWarningMessage ( settings->verbose, "!!! ProSHADE WARNING !!! Failed to detect some of the polyhedral symmetries, while detecting the correct dihedral angles.", "WS00030" ); return ( ret ); }
    }
...
bool ProSHADE_internal_symmetry::detectIcosahedralSymmetry ( std::vector< proshade_double* >* CSymList, proshade_double axErr )
{
    ...
    for ( proshade_unsign c5 = 0; c5 < static_cast<proshade_unsign> ( C5List.size() ); c5++ )
    {
        for ( proshade_unsign cSym = 0; cSym < static_cast<proshade_unsign> ( CSymList->size() ); cSym++ )
        {
            ...
            dotProduct                        = ProSHADE_internal_maths::computeDotProduct ( &CSymList->at(C5List.at(c5))[1], &CSymList->at(C5List.at(c5))[2], &CSymList->at(C5List.at(c5))[3], &CSymList->at(cSym)[1], &CSymList->at(cSym)[2], &CSymList->at(cSym)[3] );
